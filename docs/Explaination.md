...
**Evaluate_Model_with_Alpha_Beta**提供新模型，游戏，进行评估的局数，是否写入Tensorboard。然后进行测试，得到结果获取分数，然后记录。我们设置2层alphabeta树的分数为1500.
...
# ui
## run.py
有GUI显示模型相互对弈以及我们和模型对弈。
## viewer.py
在Arena对弈时每得到一步会调用观察器做处理，包括显示，而人类还需要给出动作，然后在Arena继续执行。
**__init__**，保存游戏，显示的棋盘的尺寸，显示时为了能够看清步骤可设定延迟，按一下空格下一步的标志，是否记录视频的标志，视频的输出路径，帧率设置。设置屏幕，设置其标题，帧率，字体。维护的几个值，包括最后一次移动，合法落点，已选的起点。是否要等待人类输入，记录人类输入动作，是否正在运行。
**_open_recorder**，根据不同格式使用不同视频写入器。
**_close_recorder**停止写入，保存
**_capture_if_recording**记录图像
**toggle_record**翻转记录器状态
**snapshot_png**保存这一帧为图片
**_board_to_xy**像素坐标转化为棋盘坐标
**_draw_text**渲染文字
**_draw_board**画棋盘，王座，棋子，特殊提示框
**on_end**末端，画图，显示文字，刷新到屏幕记录一秒，停止记录，监听按键
**_draw_hud**写入文本情况信息
**get_human_action**人类输入先初始化值，得到合法动作，监听按键和鼠标，根据按键和点击位置更新变量值，更新画面，记录画面
**on_step**根据棋盘画面，输入的动作，谁在下，监听键盘，更新画面，文本，刷新，记录，看是否等待。
**HumanGUIPlayer**获取观察，输入棋盘会调用观察得到人类动作
